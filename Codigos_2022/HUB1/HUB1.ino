...
//---------------------------------CONFIGURAÇÃO DO HUB--------------------------------------
// Variáveis da Rede
uint16_t this_node = HUB1; // Endereço deste Nó
...
bool status_sensores[5];
const unsigned long interval = 125; // Intervalo de checagem dos sensores
...
bool sendSMessage(uint16_t to)
{
  ...
  //*
  Serial.print("HUB1: ");
  Serial.print(status_sensores[0]);
  Serial.print("   ");
  Serial.print(status_sensores[1]);
  Serial.print("   ");
  Serial.print(status_sensores[2]);
  Serial.print("   ");
  Serial.print(status_sensores[3]);
  Serial.print("   ");
  Serial.println(status_sensores[4]); // Depuração

  return network.write(header, &status_sensores, sizeof(status_sensores));
}
...
void UpdateStatus(RF24NetworkHeader &header)
{
  ...
  if (message)
  {
    ...
    {
    case CP0:
      status_sensores[0] = true;
      time_counter_1 = 0;
      digitalWrite(LED_1, HIGH);
      break;

    case CP1:
      status_sensores[1] = true;
      time_counter_2 = 0;
      digitalWrite(LED_2, HIGH);
      break;

    case CP2:
      status_sensores[2] = true;
      time_counter_3 = 0;
      digitalWrite(LED_3, HIGH);
      break;

    case CP3:
      status_sensores[3] = true;
      time_counter_4 = 0;
      digitalWrite(LED_4, HIGH);
      break;

    case CP4:
      status_sensores[4] = true;
      time_counter_5 = 0;
...
void setup()
{
  ...
  Serial.println("DEPURACAO HUB1"); // Depuração

  status_sensores[0] = false;
  status_sensores[1] = false;
  status_sensores[2] = false;
  status_sensores[3] = false;
  status_sensores[4] = false;
