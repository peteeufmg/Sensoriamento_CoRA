...
bool status_ultrassonicos1[5];
bool status_ultrassonicos2[5];
const unsigned long interval = 500; //Intervalo de checagem do ultrassônico
...
void InformStatus(){
      ...
      Serial.print('1');
      Serial.print('C');
      Serial.print(status_ultrassonicos1[0]);
      Serial.print(status_ultrassonicos1[1]);
      Serial.print(status_ultrassonicos1[2]);
      Serial.print(status_ultrassonicos1[3]);
      Serial.print(status_ultrassonicos1[4]);
      Serial.print(status_ultrassonicos2[0]);
      Serial.print(status_ultrassonicos2[1]);
      Serial.print(status_ultrassonicos2[2]);
      Serial.print(status_ultrassonicos2[3]);
      Serial.print(status_ultrassonicos2[4]);
      
      if(status_ultrassonicos1[0]){CRC = CRC + 1;}
      if(status_ultrassonicos1[1]){CRC = CRC + 1;}
      if(status_ultrassonicos1[2]){CRC = CRC + 1;}
      if(status_ultrassonicos1[3]){CRC = CRC + 1;}
      if(status_ultrassonicos1[4]){CRC = CRC + 1;}
      if(status_ultrassonicos2[0]){CRC = CRC + 1;}
      if(status_ultrassonicos2[1]){CRC = CRC + 1;}
      if(status_ultrassonicos2[2]){CRC = CRC + 1;}
      if(status_ultrassonicos2[3]){CRC = CRC + 1;}
      if(status_ultrassonicos2[4]){CRC = CRC + 1;}
      
        if(CRC == 5){Serial.println("05");}
        if(CRC == 6){Serial.println("06");}
        if(CRC == 7){Serial.println("07");}
        if(CRC == 8){Serial.println("08");}
        if(CRC == 9){Serial.println("09");}
        if(CRC == 10){Serial.println("10");}
        if(CRC == 11){Serial.println("11");}
        if(CRC == 12){Serial.println("12");}
        if(CRC == 13){Serial.println("13");}
        if(CRC == 14){Serial.println("14");}
        if(CRC == 15){Serial.println("15");}
        if(CRC == 16){Serial.println("16");}
}
...
void UpdateStatus(RF24NetworkHeader& header){
  ...
  switch(from){
    case HUB1:
      network.read(header, &status_ultrassonicos1, sizeof(status_ultrassonicos1));
      break;
        
    case HUB2:
      network.read(header, &status_ultrassonicos2, sizeof(status_ultrassonicos2));
      break;     
    }
}
